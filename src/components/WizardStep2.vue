<template>
  <div class="flex flex-col gap-2">
    <label for="username">Email:</label>
    <input type="text" name="email" id="email" v-model="formData.email" class="bg-gray-200 border border-gray-300 rounded px-2 py-1">
    <div v-if="error" class="text-sm text-red-500">Invalid email address.</div>
  </div>
</template>
<script>
import { computed, ref, toRef } from "vue"
export default {
  expose: ['validateStep'],
  props: {
    modelValue: { type: Object },
  },
  setup(props) {
    const formData = toRef(props, 'modelValue')
    const error = ref(false)

    const validateStep = function() {
      error.value = false
      if (/(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:(2(5[0-5]|[0-4][0-9])|1[0-9][0-9]|[1-9]?[0-9]))\.){3}(?:(2(5[0-5]|[0-4][0-9])|1[0-9][0-9]|[1-9]?[0-9])|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])/.test(formData.value.email)) { return true }
      error.value = true
      return false
    }
    return { formData, validateStep, error }
  },
}
</script>